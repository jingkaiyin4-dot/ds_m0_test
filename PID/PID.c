    ...
    const DriveController *drive)
{
    ...
    /** duty × 输出方向修正 → 实际PWM */
    Motor_SetDuty(driveWheel->motor, (int16_t) lroundf(wheel->duty * motorOutputSign));
}
...
void double_pid(void)
{
    ...
    /** 2. 左轮双环PID */
    WheelController_Update(&g_driveController.left, &g_driveController.driveBase.left,
        g_driveController.leftDirectionSign, g_driveController.leftMotorOutputSign, &g_driveController);

    /** 3. 右轮双环PID */
    WheelController_Update(&g_driveController.right, &g_driveController.driveBase.right,
        g_driveController.rightDirectionSign, g_driveController.rightMotorOutputSign, &g_driveController);
}