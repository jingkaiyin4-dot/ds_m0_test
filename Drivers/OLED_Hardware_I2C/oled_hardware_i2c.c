...
static uint8_t g_oledAddress = 0x3C;
...
void OLED_WR_Byte(uint8_t dat,uint8_t mode)
{
    ...
    uint8_t addrTry[2] = {g_oledAddress, (g_oledAddress == 0x3C) ? 0x3D : 0x3C};
    uint8_t tryIndex;
    ...
    for (tryIndex = 0; tryIndex < 2; tryIndex++) {
        ...
        DL_I2C_startControllerTransfer(I2C_OLED_INST, addrTry[tryIndex], DL_I2C_CONTROLLER_DIRECTION_TX, 2);
        ...
        if (DL_I2C_getRawInterruptStatus(I2C_OLED_INST, DL_I2C_INTERRUPT_CONTROLLER_TX_DONE)) {
            g_oledAddress = addrTry[tryIndex];
            return;
        }
    }
}
